...
public class JuniperEvpnTracerouteTest {
  ...
  public void testTracerouteFromIrb100() throws IOException {
    ...
    for (Map.Entry<Flow, List<Trace>> entry : traces.entrySet()) {
      ...
      for (Trace trace : traceList) {
        ...
        assertTrue("Expected successful disposition but got: " + trace.getDisposition(),
    ...
    for (Map.Entry<String, Configuration> e : configs.entrySet()) {
      for (Map.Entry<String, Vrf> vrfEntry : e.getValue().getVrfs().entrySet()) {
        for (Layer2Vni vni : vrfEntry.getValue().getLayer2Vnis().values()) {
          System.out.printf("L2VNI %s/%s: vni=%d vlan=%d src=%s bum=%s floodList=%s%n",
              e.getKey(), vrfEntry.getKey(), vni.getVni(), vni.getVlan(),
              vni.getSourceAddress(), vni.getBumTransportMethod(),
  ...
  public void testModel() throws IOException {
    Batfish batfish = loadTestrigAndComputeDataPlane();
    VIModelQuestionPlugin.VIModelQuestion question = new VIModelQuestionPlugin.VIModelQuestion(null);
    ...
    assertTrue(
        "VIModel should contain node '" + DRT_HOSTNAME + "'. Actual: " + viAnswer.getNodes().keySet(),
    ...
    assertEquals("Layer2 VNIs should have unique keys", layer2Vnis.size(), layer2Vnis.keySet().size());
    assertEquals("Layer3 VNIs should have unique keys", layer3Vnis.size(), layer3Vnis.keySet().size());
  }
